...
displaymenu:
        ...
        ofstream filedorder;                        /* STORE ORDER FROM CUSTOMER INTO ORDER.TXT*/
        filedorder.open("d_order.txt", ios::app);

        ofstream filetorder;                        /* STORE ORDER FROM CUSTOMER INTO ORDERTEMP.TXT*/
                ...
                if (order_arr[c] >= 1 && order_arr[c] <= 15){
                   quantity:
                    ...
                    if (quan[c] >= 1 && quan[c] <= 10){
                        if (cust_type == 1){
                            filedorder << " " << order_arr[c] << " " << quan[c];       /* IF INPUT ARE ALL CORRECT, THEN STORE DINE IN ORDER INTO D_ORDER.TXT*/
                        }else if (cust_type == 2){
                            filetorder << "  " << order_arr[c] << " " << quan[c];      /* IF INPUT ARE ALL CORRECT, THEN STORE TAKE AWAY ORDER INTO T_ORDER.TXT*/
                        }
                        c++;                                                    /* ORDER +1 */
                    }else{
                        SetConsoleTextAttribute(h,4);
...
void Customer::add_order()
{
    ...
    do                                   /* GET PRODUCT NO AND QUANTITY FROM CUSTOMER */
    {
        ...
        if (order_arr[c] >= 1 && order_arr[c] <= 15){
            ...
            if (quan[c] >= 1 && quan[c] <= 10){
                if (cust_type == 1){
                    filedorder << " " << cust_type << " " << table_no << " " << order_arr[c] << " " << quan[c];      /* IF INPUT ARE ALL CORRECT, THEN STORE INTO ORDER.TXT*/
                }else if(cust_type == 2){
                    filetorder << " " << cust_type << " " << cust_name << " " << cust_phone << " " << order_arr[c] << " " << quan[c];
                }
                c++;                                                    /* ORDER +1 */
            }else{
                SetConsoleTextAttribute(h,4);
...
int main() {                    /* MAIN PAGE */
    ...
    cout << "\t\t\t\t\t\t       <2> Change Order Quantity\t\t\t" << endl;
