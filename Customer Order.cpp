...
void Customer::add_order()                   /* FUNCTION FOR CUSTOMER TO ADD ORDER */
{
    ...
    {
        if (cust_type == 1){
            filedorder << " " << cust_type << " " << table_no << " " << order_arr[x] << " " << quan[x];      /* IF QUANTITY IS IN THE RANGE, THEN STORE DINE IN ORDER INTO D_ORDER.TXT*/
        }else if(cust_type == 2){
            filetorder << " " << cust_type << " " << cust_name << " " << cust_phone << " " << order_arr[x] << " " << quan[x];       /* IF QUANTITY IS IN THE RANGE, THEN STORE DINE IN ORDER INTO T_ORDER.TXT*/
        }
    }
...
void Customer::cancel_order()
{
    ...
    if(found == 1)
    {
        ...
        if (cust_type == 1){
            filedorder << " " << cust_type << " " << table_no << " " << order_arr[i] << " " << quan[i];      /* IF QUANTITY IS IN THE RANGE, THEN STORE DINE IN ORDER INTO D_ORDER.TXT*/
        }else if(cust_type == 2){
            filetorder << " " << cust_type << " " << cust_name << " " << cust_phone << " " << order_arr[i] << " " << quan[i];       /* IF QUANTITY IS IN THE RANGE, THEN STORE DINE IN ORDER INTO T_ORDER.TXT*/
        }
